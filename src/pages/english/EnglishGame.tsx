...
import { ArrowLeft, Book, Languages, Trophy } from 'lucide-react';
...
export const EnglishGame = () => {
    ...
    useEffect(() => {
        if (selectedLevelId) {
            const level = englishData.levels.find(l => l.id === selectedLevelId);
            if (level) {
                setCurrentLevel(level);
                setQuestions([...level.questions].sort(() => Math.random() - 0.5));

                // Show lecture if available, otherwise start game
                if (level.lecture) {
                    setShowLecture(true);
                    setGameStatus('playing'); // We prepare the game but show modal over it
                } else {
    ...
    const handleAnswer = (option: string | number) => {
        ...
        if (isCorrect) {
            ...
            confetti({
                ...
                spread: 70,
                origin: { y: 0.6 },
                colors: ['#6366f1', '#ffffff'] // Indigo and White
            });
                ...
                <div className="max-w-6xl mx-auto">
                    <div className="flex items-center justify-between mb-8">
                        ...
                        <div className="flex items-center gap-4">
                            <GlassCard className="px-4 py-2 flex items-center gap-2 !rounded-full bg-indigo-500/10 border-indigo-500/30">
                                <Languages className="text-indigo-400" size={20} />
                                <span className="text-indigo-100 font-bold">{englishData.title}</span>
                            </GlassCard>
                        </div>
                    </div>
                            ...
                            return (
                                <GlassCard
                                    ...
                                    className={`group relative overflow-hidden transition-all duration-300 ${isLocked
                                        ? 'opacity-60 grayscale border-white/5 cursor-not-allowed'
                                        : 'cursor-pointer border-indigo-500/20 hover:border-indigo-500/50'
                                        }`}
                                >
                                    ...
                                    <div className="flex flex-col h-full relative z-10">
                                        ...
                                        <h3 className={`text-xl font-bold mb-2 transition-colors ${isLocked ? 'text-gray-400' : 'text-white group-hover:text-indigo-400'
                                            }`}>
                                            {level.title}
                                        </h3>
                                        <p className="text-sm text-gray-400 mb-4 flex-grow">{level.description}</p>
                                        <div className="flex items-center gap-2 text-xs text-indigo-300 bg-indigo-900/20 w-fit px-2 py-1 rounded-md">
                                            <Trophy size={12} />
    ...
    return (
        <div className="min-h-screen bg-[radial-gradient(ellipse_at_center,_var(--tw-gradient-stops))] from-indigo-900/20 via-[#0b0c15] to-black flex items-center justify-center p-4">
            ...
            <LectureModal
                ...
                onComplete={() => setShowLecture(false)}
            />
            ...
            {/* Main Game Content (blur when lecture is open) */}
            <div className={`max-w-4xl w-full transition-all duration-500 ${showLecture ? 'blur-lg opacity-50' : ''}`}>
                <div className="flex justify-between items-center mb-8 bg-black/40 backdrop-blur-md p-4 rounded-2xl border border-white/5">
                    <div className="flex items-center gap-4">
                        <NeonButton variant="blue" size="sm" onClick={returnToMenu}>
                            <ArrowLeft size={16} />
                        </NeonButton>
                        <div className="text-indigo-400 font-display tracking-wider">
                ...
                <AnimatePresence mode='wait'>
                    <motion.div
                        ...
                        className="w-full"
                    >
                        <GlassCard className="p-8 md:p-12 relative overflow-hidden border-indigo-500/20">

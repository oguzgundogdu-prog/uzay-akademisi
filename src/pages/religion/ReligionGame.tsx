...
export const ReligionGame = () => {
    ...
    // Derived State
    const currentLevel: Level | undefined = religionData.levels.find(l => l.id === selectedLevelId);
    ...
    const startGame = (levelId: string) => {
        const level = religionData.levels.find(l => l.id === levelId);
        if (!level) return;
    ...
    const handleAnswer = (option: string | number) => {
        ...
        if (isCorrect) {
            ...
            if (currentStreak > 0 && currentStreak % 3 === 0) {
                const messages = ["MAŞALLAH!", "BİLGİN!", "NURLU!", "SÜPER!"];
    ...
    if (gameStatus === 'playing' && currentQuestion) {
        return (
            <div className="min-h-screen bg-[radial-gradient(ellipse_at_center,_var(--tw-gradient-stops))] from-cyan-900/20 via-[#0b0c15] to-black py-8 px-4 flex flex-col items-center">
                <AnimatePresence>
                    {showMotivation && (
                        <motion.div
                            ...
                            className="fixed left-1/2 top-1/2 -translate-x-1/2 font-display font-black text-cyan-500 drop-shadow-[0_0_20px_#22d3ee] z-[100] text-4xl pointer-events-none"
                        >
                            {showMotivation}
                        </motion.div>
                    )}
                ...
                <div className="max-w-4xl w-full space-y-8">
                    ...
                    <div className="flex justify-between items-center bg-black/40 backdrop-blur-md p-4 rounded-2xl border border-white/5 sticky top-0 z-50">
                        <NeonButton variant="cyan" size="sm" onClick={() => setGameStatus('menu')} className="gap-2 shrink-0">
                            <ArrowLeft size={16} />
                            <span className="hidden sm:inline">VAZGEÇ</span>
                        </NeonButton>

                        <div className="flex-1 mx-4 sm:mx-8 flex flex-col gap-1">
                            <div className="flex justify-between text-[10px] sm:text-xs text-cyan-400/70 font-mono font-bold">
                                <span>COMBO: {currentStreak}x</span>
                                <span>{currentQuestionIndex + 1} / {totalQuestions}</span>
                            </div>
                            <div className="h-3 bg-white/5 rounded-full overflow-hidden border border-white/10">
                                <motion.div
                                    className="h-full bg-gradient-to-r from-cyan-500 to-blue-600 shadow-[0_0_10px_#22d3ee]"
                                    initial={{ width: 0 }}
                        ...
                        <div className="flex items-center gap-2 bg-cyan-500/10 px-3 py-1.5 rounded-xl border border-cyan-500/30">
                            <div className="animate-pulse text-cyan-500">❤️</div>
                            <span className="font-bold text-white font-mono text-lg">{hearts}</span>
                        </div>
                    </div>

                    <GlassCard className="p-8 md:p-12 relative overflow-visible border-cyan-500/20 min-h-[450px] flex flex-col justify-center text-center">
                        <AnimatePresence>
                            {lastBonus > 0 && (
                                <motion.div
                                    ...
                                    exit={{ opacity: 0 }}
                                    className="absolute top-4 right-4 text-cyan-400 font-mono font-bold"
                                >
                        ...
                        {feedback === 'wrong' && currentQuestion.explanation && (
                            ...
                            >
                                <p className="text-cyan-200 font-medium italic">💡 {currentQuestion.explanation}</p>
                            </motion.div>
                        )}
                        ...
                        {!feedback && (
                            <div className="mt-12">
                                <NeonButton
                                    ...
                                    className="gap-2"
                                >
                                    <Sparkles size={16} /> 50:50 ({isFiftyFiftyUsed ? 'KULLANILDI' : '5 💎'})
                                </NeonButton>
                            </div>
                        )}
                    </GlassCard>
            ...
            <div className="max-w-6xl mx-auto">
                <header className="text-center space-y-4 mb-12">
                    <motion.div
                        ...
                        animate={{ scale: 1, opacity: 1 }}
                        className="inline-block p-4 rounded-3xl bg-cyan-500/20 border-2 border-cyan-500/50 mb-4"
                    >
                        <Moon className="text-cyan-400 w-12 h-12" />
                    </motion.div>
                    <h1 className="text-6xl font-black font-display text-white italic drop-shadow-[0_0_15px_rgba(6,182,212,0.5)]">
                        DİNİ <span className="text-cyan-500">BİLGİLER</span>
                    </h1>
                    <p className="text-xl text-cyan-200/70 font-medium tracking-wide">
                        Bilgini sına, yıldızları topla!
                    </p>
                        ...
                        return (
                            ...
                            >
                                <GlassCard className={cn(
                                    ...
                                    isLocked ? "border-white/5 opacity-50" : "border-cyan-500/20 hover:border-cyan-500/50 bg-cyan-900/10"
                                )}>
                                    <div className="flex justify-between items-start mb-6">
                                        <div className={cn(
                                            "w-16 h-16 rounded-2xl flex items-center justify-center text-3xl shadow-lg border-2",
                                            isLocked ? "bg-white/5 border-white/10 text-white/20" : "bg-cyan-500/20 border-cyan-500/30 text-cyan-400"
                                        )}>
                                            {isLocked ? <div className="i-lucide-lock w-7 h-7" /> : <span>{index + 1}</span>}
                                        </div>
                                        <div className="flex flex-col items-end gap-1">
                                            <div className="px-3 py-1 rounded-full bg-cyan-400/10 border border-cyan-400/30 text-cyan-400 text-xs font-bold">
                                    ...
                                    <p className="text-cyan-200/60 mb-8 flex-grow leading-relaxed">
                                        {level.description}
                                    </p>
                                    ...
                                    {isLocked ? (
                                        ...
                                        <NeonButton
                                            ...
                                            className="gap-2 shadow-[0_4px_20px_rgba(6,182,212,0.2)]"
                                        >
                                            KEŞFETMEYE BAŞLA
                                        </NeonButton>
                                    )}

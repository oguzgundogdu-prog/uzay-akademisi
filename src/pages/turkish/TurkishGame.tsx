...
export const TurkishGame = () => {
    ...
    // Derived State
    const currentLevel: Level | undefined = turkishData.levels.find(l => l.id === selectedLevelId);
    ...
    const startGame = (levelId: string) => {
        const level = turkishData.levels.find(l => l.id === levelId);
        if (!level) return;
    ...
    const handleAnswer = (option: string | number) => {
        ...
        if (isCorrect) {
            ...
            if (currentStreak > 0 && currentStreak % 3 === 0) {
                const messages = ["HARİKA!", "KELİME AVCI!", "HIZLI OKUR!", "SÜPER STAR!"];
    ...
    if (gameStatus === 'playing' && currentQuestion) {
        return (
            <div className="min-h-screen bg-[radial-gradient(ellipse_at_center,_var(--tw-gradient-stops))] from-red-900/20 via-[#0b0c15] to-black py-8 px-4 flex flex-col items-center">
                <AnimatePresence>
                    {showMotivation && (
                        <motion.div
                            ...
                            className="fixed left-1/2 top-1/2 -translate-x-1/2 font-display font-black text-red-500 drop-shadow-[0_0_20px_#ef4444] z-[100] text-4xl pointer-events-none"
                        >
                            {showMotivation}
                        </motion.div>
                    )}
                ...
                <div className="max-w-4xl w-full space-y-8">
                    ...
                    <div className="flex justify-between items-center bg-black/40 backdrop-blur-md p-4 rounded-2xl border border-white/5 sticky top-0 z-50">
                        <NeonButton variant="red" size="sm" onClick={() => setGameStatus('menu')} className="gap-2 shrink-0">
                            <ArrowLeft size={16} />
                            <span className="hidden sm:inline">VAZGEÇ</span>
                        </NeonButton>

                        <div className="flex-1 mx-4 sm:mx-8 flex flex-col gap-1">
                            <div className="flex justify-between text-[10px] sm:text-xs text-red-400/70 font-mono font-bold">
                                <span>STREAK: {currentStreak}x</span>
                                <span>{currentQuestionIndex + 1} / {totalQuestions}</span>
                            </div>
                            <div className="h-3 bg-white/5 rounded-full overflow-hidden border border-white/10">
                                <motion.div
                                    className="h-full bg-gradient-to-r from-red-500 to-rose-600 shadow-[0_0_10px_#ef4444]"
                                    initial={{ width: 0 }}
                    ...
                    <GlassCard className="p-8 md:p-12 relative overflow-visible border-red-500/20 min-h-[450px] flex flex-col justify-center text-center">
                        <AnimatePresence>
                            {lastBonus > 0 && (
                                <motion.div
                                    ...
                                    exit={{ opacity: 0 }}
                                    className="absolute top-4 right-4 text-red-400 font-mono font-bold"
                                >
                        ...
                        {!feedback && (
                            <div className="mt-12">
                                <NeonButton
                                    ...
                                    className="gap-2"
                                >
                                    <Star size={16} /> 50:50 ({isFiftyFiftyUsed ? 'KULLANILDI' : '5 💎'})
                                </NeonButton>
                            </div>
                        )}
                    </GlassCard>
    ...
    return (
        <div className="min-h-screen bg-[radial-gradient(ellipse_at_top,_var(--tw-gradient-stops))] from-red-900/40 via-[#0f1020] to-[#050608] p-4 md:p-8">
            <div className="max-w-6xl mx-auto">
                <header className="text-center space-y-4 mb-12">
                    <motion.div
                        initial={{ scale: 0.5, opacity: 0 }}
                        animate={{ scale: 1, opacity: 1 }}
                        className="inline-block p-4 rounded-3xl bg-red-500/20 border-2 border-red-500/50 mb-4"
                    >
                        <BookOpen className="text-red-400 w-12 h-12" />
                    </motion.div>
                    <h1 className="text-6xl font-black font-display text-white italic drop-shadow-[0_0_15px_rgba(239,68,68,0.5)]">
                        TÜRKÇE <span className="text-red-500">YILDIZI</span>
                    </h1>
                    <p className="text-xl text-red-200/70 font-medium tracking-wide">
                        Kelimelerin büyülü dünyasında bir yolculuğa çıkmaya ne dersin?
                    </p>
                    <div className="flex justify-center mt-6">
                        <NeonButton variant="red" size="sm" onClick={() => navigate('/')}>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
                    {turkishData.levels.map((level, index) => {
                        ...
                        return (
                            ...
                            >
                                <GlassCard className={cn(
                                    ...
                                    isLocked ? "border-white/5 opacity-50" : "border-red-500/20 hover:border-red-500/50 bg-red-900/10"
                                )}>
                                    <div className="flex justify-between items-start mb-6">
                                        <div className={cn(
                                            "w-16 h-16 rounded-2xl flex items-center justify-center text-3xl shadow-lg border-2",
                                            isLocked ? "bg-white/5 border-white/10 text-white/20" : "bg-red-500/20 border-red-500/30 text-red-400"
                                        )}>
                                            {isLocked ? <div className="i-lucide-lock w-7 h-7" /> : <span>{index + 1}</span>}
                                        </div>
                                        <div className="flex flex-col items-end gap-1">
                                            <div className="px-3 py-1 rounded-full bg-red-400/10 border border-red-400/30 text-red-400 text-xs font-bold">
                                    ...
                                    <p className="text-red-200/60 mb-8 flex-grow leading-relaxed">
                                        {level.description}
                                    </p>
                                    ...
                                    {isLocked ? (
                                        ...
                                        <NeonButton
                                            ...
                                            className="gap-2 shadow-[0_4px_20px_rgba(239,68,68,0.2)]"
                                        >
                                            KEŞFETMEYE BAŞLA
                                        </NeonButton>
                                    )}
